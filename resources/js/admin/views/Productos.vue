...
async function loadSucursales() {
    ...
    sucursales.value = data.filter(s => s.activa !== false);
    if (sucursales.value.length) selectedSucursal.value = sucursales.value[0].id;
}
...
async function loadProductos() {
    ...
    loading.value = true;
    try {
        const { data } = await axios.get(`/api/admin/sucursales/${selectedSucursal.value}/productos`);
...
async function save() {
    ...
    const url = editingId.value
        ? `/api/admin/sucursales/${selectedSucursal.value}/productos/${editingId.value}`
        : `/api/admin/sucursales/${selectedSucursal.value}/productos`;
...
async function remove(p) {
    ...
    try {
        await axios.delete(`/api/admin/sucursales/${selectedSucursal.value}/productos/${p.id}`);
...
const sucursalNombre = computed(() =>
    sucursales.value.find(s => s.id === selectedSucursal.value)?.nombre ?? ''
);
...
<template>
  <div>
    ...
    <div class="flex gap-1.5 mb-4 flex-wrap">
      <button v-for="s in sucursales" :key="s.id" @click="selectedSucursal = s.id"
        ...
        <table class="w-full text-xs min-w-[700px]">
          <thead class="bg-gray-50 text-gray-400 uppercase border-b border-gray-100">
            <tr>
              ...
              <th class="px-3 py-2 text-center font-semibold tracking-wide hidden xl:table-cell">Stock Min/Max</th>
          ...
          <tbody>
            ...
            <tr v-for="p in filtered" :key="p.id"
              ...
              <td class="px-3 py-1.5 text-right text-gray-500 hidden lg:table-cell">
                {{ p.precio_mayoreo > 0 ? fmtBs(p.precio_mayoreo) : '—' }}
              </td>
              <td class="px-3 py-1.5 text-right text-gray-500 hidden lg:table-cell">{{ fmtBs(p.precio_compra) }}</td>
              <td class="px-3 py-1.5 text-center text-gray-500 hidden xl:table-cell">
                {{ p.stock_minimo ?? 0 }} / {{ p.stock_maximo ?? 100 }}
              </td>
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl">
          <h3 class="font-bold text-gray-800 text-sm">
            ...
            <span class="font-normal text-gray-400 text-xs ml-1">— {{ sucursalNombre }}</span>
          </h3>
        ...
        <form @submit.prevent="save" class="px-5 py-4 space-y-3">
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <input v-model="form.codigo_barras" type="text" placeholder="Ej: 7772106..."
                class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
              <p v-if="errors.codigo_barras" class="text-red-500 text-xs mt-0.5">{{ errors.codigo_barras[0] }}</p>
            </div>
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <input v-model="form.precio_compra" type="number" step="0.01" min="0" required placeholder="0.00"
                class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              ...
              <input v-model="form.precio_venta" type="number" step="0.01" min="0" required placeholder="0.00"
                class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              ...
              <input v-model="form.precio_mayoreo" type="number" step="0.01" min="0" placeholder="0.00"
                class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input v-model="form.stock_minimo" type="number" min="0" placeholder="0"
                class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1">Stock Máximo</label>
              <input v-model="form.stock_maximo" type="number" min="0" placeholder="100"
                class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <label for="activo" class="text-xs font-semibold text-gray-600">Producto activo</label>
          </div>
