...
async function remove(s) {
    if (!confirm(`¿Eliminar la sucursal "${s.nombre}"?\n\nEsta acción mueve la sucursal a la papelera (soft delete). Los datos de ventas e inventario se conservan.`)) return;
  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-0.5">Gestión de sucursales y sus bases de datos</p>
      </div>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 text-gray-500 text-xs uppercase border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left hidden xl:table-cell">Base de Datos</th>
        ...
        <tbody>
          ...
          <tr v-for="s in filtered" :key="s.id" class="border-t border-gray-50 hover:bg-gray-50 transition-colors">
            <td class="px-4 py-3">
              <DropdownMenu>
                <button @click="openEdit(s)" class="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                ...
                <button @click="toggle(s)" class="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                  <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                ...
                <button @click="remove(s)" class="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
            ...
            <td class="px-4 py-3 font-semibold text-gray-800">{{ s.nombre }}</td>
            <td class="px-4 py-3 text-gray-500 hidden sm:table-cell">{{ s.slug }}</td>
            <td class="px-4 py-3 text-gray-400 text-xs font-mono hidden xl:table-cell">{{ s.base_datos }}</td>
            <td class="px-4 py-3 text-gray-500 hidden md:table-cell">{{ s.telefono ?? '—' }}</td>
            <td class="px-4 py-3 text-center">
              <button @click="toggle(s)"
                :class="s.activa ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'"
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="showModal = false">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl">
          <h3 class="font-bold text-gray-800 text-base">{{ editingId ? 'Editar Sucursal' : 'Nueva Sucursal' }}</h3>
        ...
        <form @submit.prevent="save" class="px-6 py-4 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p v-if="errors.nombre" class="text-red-500 text-xs mt-1">{{ errors.nombre[0] }}</p>
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1.5">
                Slug *
                <span v-if="editingId" class="font-normal text-gray-400 ml-1">(no editable)</span>
              </label>
              <input v-model="form.slug" type="text" :disabled="!!editingId" required placeholder="ej: centro"
                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-400 disabled:cursor-not-allowed">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.telefono" type="text" placeholder="Ej: 78901234"
                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
          ...
          <div>
            ...
            <input v-model="form.direccion" type="text" placeholder="Ej: Av. Arce 123, La Paz"
              class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          </div>

          <template v-if="!editingId">
            <div class="pt-2 border-t border-gray-100">
              <p class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Administrador de la sucursal</p>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="form.admin_nombre" type="text" required placeholder="Juan Pérez"
                  class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <p v-if="errors.admin_nombre" class="text-red-500 text-xs mt-1">{{ errors.admin_nombre[0] }}</p>
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Email *</label>
                <input v-model="form.admin_email" type="email" required placeholder="admin@ejemplo.com"
                  class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <p v-if="errors.admin_email" class="text-red-500 text-xs mt-1">{{ errors.admin_email[0] }}</p>
              </div>
            </div>
            <div>
              ...
              <input v-model="form.admin_password" type="password" required placeholder="Mínimo 8 caracteres"
                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <p v-if="errors.admin_password" class="text-red-500 text-xs mt-1">{{ errors.admin_password[0] }}</p>
            </div>
          </template>
