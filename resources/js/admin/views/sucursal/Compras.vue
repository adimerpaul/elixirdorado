...
const total = computed(() =>
    cart.value.reduce((sum, i) => sum + i.cantidad * i.precio_unitario, 0)
);
...
<template>
  <div class="flex flex-col h-full">
    ...
    <!-- ══════════════ TAB: NUEVA COMPRA ══════════════ -->
    <div v-if="tab === 'nueva'" class="flex gap-4 flex-1 min-h-0">

      <!-- Panel izquierdo: productos -->
      <div class="w-80 flex-shrink-0 flex flex-col bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-3 border-b border-gray-100">
          <div class="relative">
            ...
            <input v-model="searchProd" type="text" placeholder="Buscar producto..."
              class="w-full border border-gray-200 rounded-lg pl-8 pr-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
          </div>
        </div>
        <div class="flex-1 overflow-y-auto divide-y divide-gray-50">
          ...
          <button v-for="p in prodFiltrados" :key="p.id"
            ...
            class="w-full flex items-center gap-2 px-3 py-2 hover:bg-blue-50 transition-colors text-left">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-gray-400">Stock: {{ p.stock_actual }} · Costo: {{ fmtBs(p.precio_compra) }}</p>
            </div>
